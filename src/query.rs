...
pub async fn view<'a>(
    ...
    views: &'a Vec<View>,
    pool: &bb8::Pool<bb8_redis::RedisConnectionManager>,
) -> Result<HashMap<&'a str, Value>, String> {
    let patch_map = build_patch_map(entity_id, views, pool).await?;

    views
        .par_iter()
        .map(|view| match patch_map.get(&view.field[..]) {
            Some(patches) => proj(&view, patches).map(|val| {
...
async fn build_patch_map<'a>(
    ...
    views: &'a Vec<View>,
    pool: &bb8::Pool<bb8_redis::RedisConnectionManager>,
) -> Result<HashMap<&'a str, Vec<(u64, Patch)>>, String> {
...
async fn multi<C>(conn: &mut C) -> Result<(), String>
where
...
async fn exec<C>(conn: &mut C) -> Result<(), String>
where
    C: redis::aio::ConnectionLike,
{
    redis::cmd("EXEC")
